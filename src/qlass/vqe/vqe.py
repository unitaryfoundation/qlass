...
class VQE:
    ...
    def draw_ansatz(
        ...
        **kwargs: Any,
        ...
        """
        Draw the ansatz circuit generated by this VQE executor.

        Args:
            params (np.ndarray): Parameters used to build the ansatz circuit.
            pauli_string (Optional[str]): Pauli string passed to the executor. If ``None``,
                an identity string matching the Hamiltonian size is used.
            ...
            save_path (Optional[str]): If provided, save the rendering to this path.
            backend (str): Drawing backend. Currently only ``"perceval"`` is supported.
            **kwargs (Any): Additional keyword arguments forwarded to ``qlass.utils.draw_circuit``.

        Raises:
            TypeError: If the executor does not return a Perceval ``Processor`` or ``Circuit``.
        """
        if pauli_string is None:
            pauli_string = "I" * self.num_qubits

        circuit_or_processor = self.executor(params, pauli_string)
        draw_circuit(
