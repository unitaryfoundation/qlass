...
Standard VQE using a sampling-based executor with a Hartree-Fock (CSF) initial state:
...
.. code-block:: python
   ...
   from qlass.quantum_chemistry import Hchain_hamiltonian_WFT
   from perceval.algorithm import Sampler
   import warnings
...
Drawing a VQE Ansatz Circuit
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Drawing a Perceval processor generated by a VQE ansatz:
...
.. code-block:: python
   ...
   params = np.array([0.0, 0.25, 0.5, 0.75])
   processor = le_ansatz(params, "II")
   draw_circuit(
       processor,
       output_format="mpl",
       compact=True,
       save_path="ansatz.png",
       recursive=True,
   )
...
.. code-block:: python
   ...
   from qlass.quantum_chemistry import Hchain_KS_hamiltonian
   from perceval.algorithm import Sampler
...
.. code-block:: python
   ...
   def executor(params, pauli_string):
       processor = Bitstring_initial_states(1, n_orbs, params, pauli_string, cost="VQE")
       sampler = Sampler(processor)
       samples = sampler.samples(10_000)
       return samples
